...
import { getHashTarget, getGameForHash } from './nav-hashes.mjs';
...
/**
 * Game-section hashes stay pending until the target section is visible.
 *
 * @param {Element} target
 * @param {string} hash
 * @returns {boolean}
 */
function isHashTargetReady(target, hash) {
  if (!getGameForHash(hash)) {
    return true;
  }
...
export function restoreHashScroll() {
  ...
  if (!isHashTargetReady(target, hash)) {
    hashRestoreAttempts += 1;
    if (hashRestoreAttempts < MAX_HASH_RESTORE_ATTEMPTS) {
      setTimeout(restoreHashScroll, HASH_RESTORE_RETRY_MS);
    }
