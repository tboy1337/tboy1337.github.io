...
const GAME_MENU_READY_MAX_ATTEMPTS = 60;
  ...
  if (typeof window.switchPortfolioGame !== 'function') {
    if (attempt < GAME_MENU_READY_MAX_ATTEMPTS) {
      return new Promise((resolve) => {
        setTimeout(() => {
          activateGameForHash(attempt + 1).then(resolve);
        }, GAME_MENU_READY_RETRY_MS);
      });
    }
    return Promise.resolve(false);
  }
...
export { shouldLoadGamesImmediately, showGamesLoadError };

/** Clears the in-flight games bundle promise (unit tests only). */
export function resetGamesLoadStateForTests() {
