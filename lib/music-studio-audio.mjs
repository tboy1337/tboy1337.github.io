...
export function createMusicStudioAudioEngine(options = {}) {
  ...
  const EFFECT_REBUILD_MAX_WAIT_MS = 3000;
  ...
  function scheduleEffectRebuild() {
    ...
    const attemptRebuild = () => {
      effectRebuildTimer = null;
      if (activeVoices.length > 0 && Date.now() - waitStartedAt < EFFECT_REBUILD_MAX_WAIT_MS) {
        effectRebuildTimer = setTimeout(attemptRebuild, EFFECT_REBUILD_DEBOUNCE_MS);
        return;
      }
      rebuildEffectChain();
    };
