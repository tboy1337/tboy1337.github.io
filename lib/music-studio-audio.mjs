...
export function applyEffectSettings(handles, effects) {
  if (handles.filter && effects.filter.enabled) {
    ...
    handles.filter.node.frequency.value = effects.filter.frequency;
 ...
 * @param {AudioContext} context
 * @param {EffectSettings} effects
 * @returns {{ nodes: AudioNode[]; chorusLFO: OscillatorNode | null; output: AudioNode; handles: EffectChainHandles }}
 */
export function buildSharedEffectChain(context, effects) {
  ...
  return { nodes, chorusLFO, output, handles };
}
