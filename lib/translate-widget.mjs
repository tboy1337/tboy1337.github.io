...
/**
 * Selectors for Google Translate chrome that is not the language dropdown.
 * The widget itself lives in #google_translate_element and must be left intact.
 */
export const TRANSLATE_BANNER_SELECTORS = [
 ...
 * @returns {boolean}
 */
function isTranslateWidgetElement(element) {
  return Boolean(element.closest('#google_translate_element'));
}

/**
 * Hide the top translate banner and related Google chrome without removing
 * those nodes — Google needs the banner iframe in the DOM to finish translating.
 *
 * @param {ParentNode | Document} [root]
 * @returns {number} Number of chrome nodes hidden
 */
export function hideTranslateChrome(root = document) {
  const toHide = [];

  for (const selector of TRANSLATE_BANNER_SELECTORS) {
    root.querySelectorAll(selector).forEach((element) => {
      if (!isTranslateWidgetElement(element)) {
        toHide.push(element);
      }
    });
  }
  ...
  if (body) {
    for (const child of Array.from(body.children)) {
      if (!(child instanceof HTMLElement) || child.id === 'google_translate_element') {
        continue;
      }
  ...
  const uniqueNodes = [...new Set(toHide)];
  for (const node of uniqueNodes) {
    ...
    console.debug('Hiding Google Translate banner chrome', {
      tag: node.tagName,
  ...
  if (uniqueNodes.length > 0) {
    console.info('Hid Google Translate top banner chrome', {
      hidden: uniqueNodes.length
    });
  }
...
/**
 * Watch for Google re-injecting the top banner after a language is chosen.
 *
 * @returns {boolean}
 */
export function attachTranslateChromeHider() {
  hideTranslateChrome();

  if (chromeObserver || !document.body) {
    return Boolean(chromeObserver);
  }
  ...
  console.info('Observing document body for Google Translate banner chrome');
  return true;
}
